...
#define MAX_VISIBLE_ITEMS	CG_PopupNumPopups()
...
int CG_PopupNumPopups();



void CG_InitPMGraphics( void ) {
	cgs.media.pmImages[PM_DYNAMITE] =		trap_R_RegisterShaderNoMip( "gfx/limbo/cpm_dynamite" );
...
int CG_PopupNumPopups() {
	if( cg_numPopups.integer < 0 )
		return 0;
	else if( cg_numPopups.integer > 20 )
		return 20;
	else
		return cg_numPopups.integer;
}
...
================
*/
int CG_PopupFadeTime() {
	if( cg_popupFadeTime.integer < 0 )
		return 0;
	else if( cg_popupFadeTime.integer > 5000 )
		return 5000;
	else
		return cg_popupFadeTime.integer;
}
...
================
*/
int CG_PopupWaitTime() {
	if( cg_popupWaitTime.integer < 0 )
		return 0;
	else if( cg_popupWaitTime.integer > 4000 )
		return 4000;
	else
		return cg_popupWaitTime.integer;
}

// Jaybird - Customize these
#define PM_FADETIME CG_PopupFadeTime()	// 2500 default
#define PM_WAITTIME CG_PopupWaitTime()	// 2000 default

#define PM_FADETIME_BIG 1000
#define PM_WAITTIME_BIG 3500

int CG_TimeForPopup( popupMessageType_t type ) {
	// Jaybird - cg_popupTime
	if( cg_popupTime.integer < 0 )
		return 0;
	else if( cg_popupTime.integer > 2000 )
		return 2000;
	else
...
void CG_DrawPMItems( void ) {
	...
	// Jaybird - colored death shader
	switch( cg_pmWaitingList->flag ) {
		case 1:
			colourText[2] = 0.f;
			break;
		case 2:
			colourText[1] = colourText[2] = 0.f;
			break;
		default:
			break;
	}
