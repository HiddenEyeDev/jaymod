...
void CG_ShowHelp_On(int *status)
{
	...
	if(*status == SHOW_SHUTDOWN && milli < cg.fadeTime) {
		cg.fadeTime = int( 2 * milli + STATS_FADE_TIME - cg.fadeTime );
	} else if(*status != SHOW_ON) {
		cg.fadeTime = int( milli + STATS_FADE_TIME );
	}
...
void CG_DemoClick(int key, qboolean down)
{
	...
	switch(key)
	{
		...
		case K_F1:
			if(down) cgs.aviDemoRate = demo_avifpsF1.integer;
			else trap_Cvar_Set("cl_avidemo", demo_avifpsF1.string);
			return;
		case K_F2:
			if(down) cgs.aviDemoRate = demo_avifpsF2.integer;
			else trap_Cvar_Set("cl_avidemo", demo_avifpsF2.string);
			return;
		case K_F3:
			if(down) cgs.aviDemoRate = demo_avifpsF3.integer;
			else trap_Cvar_Set("cl_avidemo", demo_avifpsF3.string);
			return;
		case K_F4:
			if(down) cgs.aviDemoRate = demo_avifpsF4.integer;
...
//
#define COLOR_BG			{ 0.08f, 0.08f, 0.10f, 0.80f }
#define COLOR_BORDER		{ 0.25f, 0.25f, 0.28f, 0.35f }
#define COLOR_BG_TITLE		{ 0.10f, 0.12f, 0.16f, 0.85f }
#define COLOR_BG_VIEW		{ 0.10f, 0.12f, 0.16f, 0.85f }
#define COLOR_BORDER_TITLE	{ 0.25f, 0.25f, 0.28f, 0.30f }
#define COLOR_BORDER_VIEW	{ 0.25f, 0.25f, 0.28f, 0.30f }
#define COLOR_HDR			{ 0.78f, 0.80f, 0.84f, 1.0f }
#define COLOR_HDR2			{ 0.55f, 0.55f, 0.60f, 1.0f }
#define COLOR_TEXT			{ 0.78f, 0.80f, 0.84f,  1.0f }
...
void CG_DemoHelpDraw()
{
	if(cg.demohelpWindow == SHOW_OFF) {
		...
		if(cg.mvTotalClients > 1) {
			for (i=0; i < int( sizeof(mvhelp)/sizeof(char*) ); i++) {
				y += tSpacing;
				if(mvhelp[i] != NULL) {
